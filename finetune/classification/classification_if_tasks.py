...
class SingleOutputTask(task.Task):
    ...
    def __init__(self, config: configure_if_finetuning.FinetuningIFConfig, name,
                 tokenizer):
        super(SingleOutputTask, self).__init__(config, name)
        self._tokenizer = tokenizer

    def get_examples(self, split):
        print(self.config.raw_data_dir(self.name))
...
class FN(ClassificationTask):
    """FrameNet."""

    def __init__(self, config: configure_if_finetuning.FinetuningIFConfig, tokenizer):
        super(FN, self).__init__(config, "fn", tokenizer, [str(l) for l in list(range(54))])
